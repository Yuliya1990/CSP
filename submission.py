...
def create_chain_csp(n):
    ...
    for i, variable in enumerate(variables):
        ...
        if i==0:
            ...
            continue
        csp.add_binary_factor(oldVariable, variable, xor)
        oldVariable = variable
...
class BacktrackingSearch():
    ...
    def print_stats(self):
        ...
        for assignemtn in self.allAssignments:
            for dictValue in assignemtn:
                if assignemtn[dictValue] != None:
                    print(str(dictValue) + ' ' + str(assignemtn[dictValue]))
        ...
        if not self.mcv:
            ...
            for i, var in enumerate(self.csp.variables):
                if var not in assignment:
                    numOfConsVals = 0
                    #numOfConsVals = sum(1 for val in self.domains[var] if self.get_delta_weight(assignment,var,val) > 0)
                    for val in self.domains[var]:
                        if self.get_delta_weight(assignment, var, val) > 0:
                            numOfConsVals += 1
                    if numOfConsVals < min:
                        min = numOfConsVals
                        minIndex = i
            return self.csp.variables[minIndex]
            # END_YOUR_CODE
    ...
    def arc_consistency_check(self, var):
        ...
        while(len(li) > 0):
            variable = li.pop(0)
            for neighbor in self.csp.get_neighbor_vars(variable):
                toDelete = []
                for valNeighbor in self.domains[neighbor]:
                    notZero = False
                    for valCurrent in self.domains[variable]:
                        if self.csp.binaryFactors[variable][neighbor][valCurrent][valNeighbor] > 0:
                            notZero = True
                            break
                    if not notZero: #if binary factors return 0 for all values in domains[var]
                        toDelete.append(valNeighbor)
                if len(toDelete)>0:
                    li.append(neighbor)
                    for valueToDelete in toDelete:
                        self.domains[neighbor].remove(valueToDelete)
...
def get_sum_variable(csp, name, variables, maxSum):
    ...
    result = ('sum', name, 'agregated')
    csp.add_variable(result, [i for i in range(maxSum)])
    ...
    csp.add_variable(('sum', name, -1), [(0, 0)])

    for i, X_i in enumerate(variables):
        B_i = ('sum', name, i)
        # domain values:
        # - sum of all previous variables
        # could also add 'greater' if sum is already bigger than n
        csp.add_variable(B_i, [(i,j) for i in range(maxSum) for j in range(maxSum)])
        csp.add_binary_factor(X_i, B_i, factorXB) # Bin factor for X1-B1, X2-B2, ... Xn-Bn
        csp.add_binary_factor(('sum', name, i - 1), B_i, factorBB) # Bin factor for B1-B2, B2-B3...Bn-1-Bn


    #Binary Factor between Bn and the result
...
class SchedulingCSPConstructor():
    ...
    def add_quarter_constraints(self, csp):
        ...
        for request in self.profile.requests:
            if len(request.quarters) == 0:
                continue
            for quarter in self.profile.quarters:
                csp.add_unary_factor((request, quarter), lambda cid: cid == None or quarter in request.quarters)
    ...
    def add_unit_constraints(self, csp):
        ...
        def factorReqCourse(a, b):  # a=courseId b=unitsGiven
            if a == courseId:
                return b > 0  # If this course was chosen its units should be greater than 0
            else:
                return b == 0  # if this course was not chosen its units should be 0

        for request in self.profile.requests:
            for courseId in request.cids:

                for quarter in self.profile.quarters:
                    #csp.add_variable((courseId, quarter), [i for i in range(self.bulletin.courses[courseId].minUnits,self.bulletin.courses[courseId].maxUnits)] + [0])
                    csp.add_variable((courseId, quarter),[0,3] )
                    csp.add_binary_factor((request, quarter), (courseId, quarter), factorReqCourse)

                    if quarter not in quarterNames:
                        quarterNames.append(quarter)
                        quarterMaxUnits[quarter] = self.bulletin.courses[courseId].maxUnits
                        quarterVariables[quarter] = [(courseId, quarter)]
                    else:
                        quarterMaxUnits[quarter] += self.bulletin.courses[courseId].maxUnits
                        quarterVariables[quarter].append((courseId, quarter))

        for i, quarterName in enumerate(quarterNames):
            sumV = get_sum_variable(csp, quarterName, quarterVariables[quarterName], self.profile.maxUnits + 1)
            csp.add_unary_factor(sumV, lambda x: self.profile.minUnits <= x <= self.profile.maxUnits)
